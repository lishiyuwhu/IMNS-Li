...
with tf.device('/cpu:0'):
    ...
    def model_batch_norm(x, y_, reuse, is_train):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            net = Conv2d(net, 64, (3, 3), (1, 1), padding='SAME',
                         W_init=W_init, b_init=None, name='train_cnn1')
            net = BatchNormLayer(net, is_train, act=tf.nn.relu, name='train_batch1')
            net = MaxPool2d(net, (3, 3), (2, 2), padding='SAME', name='pool1')

            net = Conv2d(net, 64, (3, 3), (1, 1), padding='SAME',
                         W_init=W_init, b_init=None, name='train_cnn2')
            net = BatchNormLayer(net, is_train, act=tf.nn.relu, name='train_batch2')
            net = MaxPool2d(net, (3, 3), (2, 2), padding='SAME', name='pool2')

            net = Conv2d(net, 64, (3, 3), (1, 1), padding='SAME',
                         W_init=W_init, b_init=None, name='train_cnn3')
            net = BatchNormLayer(net, is_train, act=tf.nn.relu, name='train_batch3')
            net = MaxPool2d(net, (3, 3), (2, 2), padding='SAME', name='pool3')

            net = FlattenLayer(net, name='flatten')  # output: (batch_size, 2304)
            net = DenseLayer(net, n_units=1000, act=tf.nn.relu,
                             W_init=W_init2, b_init=b_init2, name='train_d1relu')  # output: (batch_size, 384)
            net = DenseLayer(net, n_units=500, act=tf.nn.relu,
                             W_init=W_init2, b_init=b_init2, name='train_d2relu')  # output: (batch_size, 192)
            net = DenseLayer(net, n_units=2, act=tf.identity,
                             W_init=tf.truncated_normal_initializer(stddev=1 / 192.0),
            ...
            L2 = 0
            for p in tl.layers.get_variables_with_name('relu', True, True):
                L2 += tf.contrib.layers.l2_regularizer(0.004)(p)
            cost = ce + L2
    ...
    with tf.device('/gpu:0'):  # <-- remove it if you don't have GPU
        network, cost, acc, train_vars= model_batch_norm(x_train_batch, y_train_batch, False, is_train=True)
        _, cost_test, acc_test, _ = model_batch_norm(x_test_batch, y_test_batch, True, is_train=False)
