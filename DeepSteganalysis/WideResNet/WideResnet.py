...
num_examples = sum(1 for _ in tf.python_io.tf_record_iterator(train_file))
...
with tf.device('/cpu:0'):
    ...
    # prepare data in cpu
    x_train_, y_train_ = read_and_decode(train_file, True)
    x_test_, y_test_ = read_and_decode(test_file, False)
    ...
    with tf.device('/gpu:0'):
        train_op = tf.train.AdamOptimizer(learning_rate, beta1=0.9, beta2=0.999,
            epsilon=1e-08, use_locking=False).minimize(cost, var_list=network.all_params[3:])
