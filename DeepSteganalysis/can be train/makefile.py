#! /usr/bin/python3
# -*- coding: utf-8 -*-
# @Time    : 2017/12/7 19:05
# @Author  : Shiyu Li
# @Software: PyCharm


import tensorflow as tf
import tensorlayer as tl
from tensorlayer.layers import *
import numpy as np
import time
from PIL import Image
import os
import io


cwd = os.getcwd()
# root = cwd + "/database/CropedBoossBase/test"
root = cwd + "/database/CropedBoossBase/train"
# TFwriter = tf.python_io.TFRecordWriter("./boossbase.test")
TFwriter = tf.python_io.TFRecordWriter("./boossbase.train")
for className in os.listdir(root):
    label = int(className[0])
    classPath = root + "/" + className + "/"
    for parent, dirnames, filenames in os.walk(classPath):
        for filename in filenames:
            imgPath = classPath + "/" + filename
            print(imgPath)
            img = Image.open(imgPath)
            print(img.size, img.mode)
            imgRaw = img.tobytes()
            example = tf.train.Example(features=tf.train.Features(feature={
                "label": tf.train.Feature(int64_list=tf.train.Int64List(value=[label])),
                "img_raw": tf.train.Feature(bytes_list=tf.train.BytesList(value=[imgRaw]))
            }))
            TFwriter.write(example.SerializeToString())

TFwriter.close()
