...
float mi_emb_calculate_lambda_from_payload(base_cost_model* m, float rel_payload, float lambda_init, float &alpha_out ) 
{
    ...
    while ( alpha3 > rel_payload ) {
        lambda3 *= 2;
        alpha3 = mi_emb_calc_average_payload( m, lambda3 ) / (m->nzAC);
        j++;
    ...
    while ( alpha1 - alpha3 > rel_payload * 1e-2 ) { // iterative search for parameter lambda
        ...
        alpha2 = 0;
        alpha2 = mi_emb_calc_average_payload( m, lambda2 ) / (m->nzAC);
...

{
    ...
    uint message_length = (uint) floor( alpha * m->nzAC );
    ...
    alpha_out = (float) (num_msg_bits[0] + num_msg_bits[1]) / (float) m->nzAC;
